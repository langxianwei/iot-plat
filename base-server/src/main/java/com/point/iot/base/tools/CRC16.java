...
public class CRC16 {  
    ...
    public static byte[] calcCrc(byte[] data, int offset, int len, int preval){
    	int ucCRCHi = (preval & 0xff00) >> 8;  
        int ucCRCLo = preval & 0x00ff;  
        int iIndex;  
        for (int i = 0; i < len; ++i) {  
        ...
        byte[] b = new byte[2];
        b[0] = (byte)(((ucCRCHi & 0x00ff)) &0xff);
        b[1] = (byte)((ucCRCLo & 0x00ff) << 8 & 0xff);
        return b;  
    }
    // 测试  
    public static void main(String[] args) {  
        // 01    03   00 00      00 05
    	byte[] crc1 = CRC16.calcCrc(new byte[]{(byte) 0x01,(byte) 0x03,(byte) 0x0A,(byte) 0x07,(byte) 0xCA,(byte) 0x02,(byte) 0xBC,(byte) 0xFF,(byte) 0x85,00,00,00,00},0,13,0xffff);
        int crc = CRC16.calcCrc16(new byte[] { 0x01, 0x03, 0x00, 0x01, (byte) 0x00, 0x01 });  
        short c = (short)crc;
